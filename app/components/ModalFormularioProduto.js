...
const ModalFormularioProduto = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(result.message || `Produto ${isEditing ? 'editado' : 'salvo'} com sucesso!`);
  ...
  const receitaForm = (
    <>
      <SelectComponent
        value={idioma}
        setValue={setIdioma}
        options={[{ value: 'ptbr', label: 'Português' }, { value: 'us', label: 'Inglês' }]}
        label={'Idioma'}
      />
    ...
    <>
      {isLoading || loadingProdutos ? (
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50' onClick={closeModal}>
          <div className='bg-white p-10 rounded-lg max-w-2xl max-h-[90vh] overflow-y-auto w-full flex flex-col gap-9' onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
              ...
              <div className='flex flex-col gap-2'>
                ...
                {arquivosParaUpload.length > 0 && (
                  <div className="text-xs text-green-600 mt-1">{arquivosParaUpload.length} nova(s) imagem(s) selecionada(s) para upload.</div>
                )}
              </div>
              ...
              <div className='flex justify-end mt-4'>
                <Button variant={'brown'} type={'submit'} label={idProduct ? 'Salvar Alterações' : 'Adicionar Produto'} disabled={isLoading} />
              </div>
            </form>
          </div>
        </div>
